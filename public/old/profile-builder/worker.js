...
onmessage = function(e) {
  ...
  for (i = 0; i < events.length; i++) {
    ...
    var price = (itemId != '') ? array["catalog-1"].data.filter(data => data._id == itemId)[0].price : '';
    ...
    var standardAttrs = ["emailAddress","sfmcContactKey","dateFirstSeen","dateJoined","dateLastSeen","autoGenerate","items","category","hasAnon","altId"];
    let attributes = {}
    for(x in events[i].user.attributes){
      if(standardAttrs.indexOf(x) == -1){
        attributes[x] = events[i].user.attributes[x]
      }
    }
    ...
    if(events[i].action == 'View Item'){
      ...
      $.ajax({
        ...
        cache: false,
        success: function(res) {
          array.results[i].tagStatus = res.status;
        },
        error: function(XMLHttpRequest, textStatus, errorThrown) {
          array.results[i].tagStatus = "Error";
        },
        async: false
      });
    } else {
