...
function validateInputs(inputs, inputObj) {
    ...
    if(!validateDemoEnvironment()){
        displayFooterMessage("userCatalogActionsFooter", "<span style='color: red;'>Please enter data into Demo Environment</span>");
        isValid = false;
    }
    if($('#ucp-user-id').val() == ''){
        displayFooterMessage("userCatalogActionsFooter", "<span style='color: red;'>Please enter a User ID</span>");
        $('#ucp-user-id').closest(".slds-form-element").addClass("slds-has-error");
        isValid = false;
    }
    if (inputs.length < 1) {
        displayFooterMessage("userCatalogActionsFooter", "No data input");
        isValid = false;
    }
    for (var i in inputs) {
        if(i == "ucp-action-num"){
            if(actionType != "Purchase"){
                if(!validateNumber(inputs[i])){
                    ...
                    displayFooterMessage("userCatalogActionsFooter", "<span style='color: red;'>Please enter only numbers into the Action Num field</span>");
                    isValid = false;
                }
            }else{
                if(!validatePurchaseNumber(inputs[i])){
                    invalidInputsArr.push(i);
                    displayFooterMessage("userCatalogActionsFooter", "<span style='color: red;'>Please check format of Purchase Number</span>");
                    isValid = false;
                }
            }
            ...
            invalidInputsArr.push(i);
            displayFooterMessage("userCatalogActionsFooter", "<span style='color: red;'>Please enter data in all fields</span>");
            isValid = false;
        }
    }
    if(isValid){
...
function validateCatalogDimensionInputs(inputs){
    ...
    if(!validateDemoEnvironment()){
        displayFooterMessage("catalogDimensionSetupFooter", "<span style='color: red;'>Please enter data into Demo Environment</span>");
        isValid = false;
    }
    if (inputs.length < 1) {
        displayFooterMessage("catalogDimensionSetupFooter", "No data input");
        isValid = false;
    }
    if($('#ucp-tag-type').val() == ''){
        $('#ucp-tag-type').closest(".slds-form-element").addClass("slds-has-error");
        displayFooterMessage("catalogDimensionSetupFooter", "<span style='color: red;'>Please enter data in all fields</span>");
        isValid = false;
    } 
    if($('#ucp-tag-ids').val() == ''){
...
function validateDemoEnvironment(){
    ...
    if($('#ucp-api-key-id').val() == ''){
        $('#ucp-api-key-id').closest(".slds-form-element").addClass("slds-has-error");
        isValid = false;
    }
    if($('#ucp-api-key-secret').val() == ''){
        $('#ucp-api-key-secret').closest(".slds-form-element").addClass("slds-has-error");
        isValid = false;
    }
    return isValid;
}
    ...
    return new Promise(
        function(resolve, reject){
            window.setTimeout(function(){
                ...
                var itemStr = '', orderStr = '';

                var countTotalPurchaseItems = 0, purchaseRowCounter = 0;
                ...
                setLocalStorage('api-key-id', $('#ucp-api-key-id').val());
                setLocalStorage('api-key-secret', $('#ucp-api-key-secret').val());
                ...
                var iterate = $("#ucp-form .ucp-user-actions-row").each(function(r, row) {
                    ...
                    var payload = {"action" : inputValues["ucp-action-id"]};

                    // Set anonymous vs. known
                    var anon = $(".slds-checkbox_on").css("display") == 'none';
                    payload.user = (anon ? {"anonId":$("#ucp-user-id").val()} : {"id":$("#ucp-user-id").val()});
                    ...
                    if (validateInputs(inputValues)) {
                        if(inputValues["ucp-action"] != "Purchase" && inputValues["ucp-action"] != "View Category"){
                            ...
                            var ucpParams = {
                                _ak:        $("#ucp-account").val(),
                                _ds:        $("#ucp-dataset").val(),
                                ".item":    { "_id": inputValues["ucp-item-id"], "type": inputValues["ucp-item-type"] },
                            ...
                            var payload = {"action" : "View Item"};
                            payload[".item"] = {"_id": inputValues["ucp-item-id"], "type": "c"};
                            payload.catalog = {"Category":{"_id": inputValues["ucp-item-id"]}};
                            payload[".category"] = {"_id": inputValues["ucp-item-id"], "type": "c"};
                    ...
                    if (validateCatalogDimensionInputs(inputValues)) {
                        ...
                        if(inputValues["ucp-tag-ids"].indexOf(';') < 0){
                            ...
                            var ucpParams = {
                                _ak:        $("#ucp-account").val(),
                                _ds:        $("#ucp-dataset").val(),
                                ".item":    { "_id": inputValues["ucp-tag-ids"], "type": "t", "tagType": $("#ucp-tag-type").val() },
                                action:     "View Tag"
                            }
                            ...
                            $.each(tagValuesArr, function(i){
                                ...
                                var ucpParams = {
                                    _ak:        $("#ucp-account").val(),
                                    _ds:        $("#ucp-dataset").val(),
                                    ".item":    { "_id": tagValuesArr[i].trim(), "type": "t", "tagType": $("#ucp-tag-type").val() },
                                    action:     "View Tag"
                                }
                                if ($("#ucp-tag-type option:selected").val() == "Category") {
                                    ucpParams[".item"]["type"] = "c";
...
function changeActionTypeOptions(v){
    ...
    var selectArr = [
        {val : "View Item", text: "View Item"},
        {val : "View Time", text: "View Time"},
    ...
    switch (v.value){
        case "c": a.closest(".slds-form-element").next().next().find("select").replaceWith(input); a.closest(".slds-form-element").next().next().next().find("input").attr("placeholder", "# times"); break;
        default: a.closest(".slds-form-element").next().next().find("input").replaceWith(selectObj); break;
    }
}
...
$(document).ready(function () {
    ...
    let cookieApiKeyId = getLocalStorage('api-key-id');
    let cookieApiKeySecret = getLocalStorage('api-key-secret');
    let cookieUserId = getLocalStorage('user-id');
    let cookieUserAnon = getLocalStorage('user-anon');
    $('#ucp-account').val(cookieAccount != '' ? cookieAccount : '');
    $('#ucp-dataset').val(cookieDataset != '' ? cookieDataset : '');
    $('#ucp-api-key-id').val(cookieApiKeyId != '' ? cookieApiKeyId : '');
    $('#ucp-api-key-secret').val(cookieApiKeySecret != '' ? cookieApiKeySecret : '');
    ...
    $("#ucp-data-add").click(function (e) {
        var newRow = $("#hidden-section-row-storage-catalog");
        var clone = newRow.clone();
        clone.find("input").val("");
        if($(e.target).closest("article").find(".ucp-user-row").length > 0){
            $(e.target).closest("article").find(".ucp-user-row").last().after(clone);
        }else{
            $(e.target).closest("article").find(".slds-card__body").last().before(clone);
        }
    ...
    $("#ucp-data-add-custom").click(function (e) {
        var newRow = $("#hidden-section-row-storage-custom");
        var clone = newRow.clone();
        clone.find("input").val("");
        if($(e.target).closest("article").find(".ucp-user-row").length > 0){
            $(e.target).closest("article").find(".ucp-user-row").last().after(clone);
